...
def get_bar_edges_image(img, start_x, start_y, bar_b=255, bar_g=255, bar_r=255, rightbar_b=255, rightbar_g=255, rightbar_r=255, tolerance=15):
    ...
    rightbar_b_tolerance = bar_b - tolerance - 1
    rightbar_g_tolerance = bar_g - tolerance - 1
    rightbar_r_tolerance = bar_r - tolerance - 1
    left_edge = None
    right_edge = None
...
def macro_loop(event=None):
    ...
    try:
        ...
        while macro_running and not fish_detected and attempts < shake_attempts:
            ...
            stable = 0
            while stable < 8:
                if pixel_search(570, 860, 1350, 1020, (fish_b, fish_g, fish_r), fish_tolerance):
                    stable += 1
                    time.sleep(0.005)
                else:
                    break
